...
const {config: {port : E2E_PORT}} = require('./src/test/aurelia/protractor.conf');
...
module.exports = {
  scripts: {
    ...
    test: {
      ...
      karma: {
        default: series(
          rimraf('target/coverage-karma'),
          'karma start src/test/aurelia/karma.conf.js'
        ),
        watch: 'karma start src/test/aurelia/karma.conf.js --no-single-run',
        debug: 'karma start src/test/aurelia/karma.conf.js --no-single-run --debug'
      },
      lint: {
        default: 'eslint src/main/aurelia',
        fix: 'eslint --fix'
      },
      all: concurrent({
    ...
    e2e: {
      ...
      protractor: {
        ...
        default: series(
          ...
          'protractor src/test/aurelia/protractor.conf.js'
        ),
        debug: series(
          'nps e2e.protractor.install',
          'protractor src/test/aurelia/protractor.conf.js --elementExplorer'
        ),
      },
      whenReady: series(
    ...
    serve: 'http-server target/dist --cors',
  },
};
